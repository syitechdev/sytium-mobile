...
class _ErrRepo extends _BaseRepo {
  ...
  Future<Result<MessagesPage>> messages(
    ...
    String? cursor,
    int limit = 50,
  }) async => const Err(ServerFailure(message: 'boom'));
}
  ...
  Future<Result<Message>> deleteForEveryone(String messageId) async {
    ...
    if (failEveryoneWith422) {
      return const Err(
        ValidationFailure(
          fieldErrors: {},
          message: 'Délai de suppression dépassé.',
        ),
      );
    }
    return Ok(
...
Widget _host(
  ...
  overrides: [
    ...
    workspaceRealtimeProvider.overrideWithValue(
      realtime ?? FakeWorkspaceRealtime(),
    ),
  ],
  child: MaterialApp(
    home: Theme(
...
void main() {
  ...
  testWidgets(
    'flags its channel as the one on screen, and releases it on close',
    (tester) async {
      ...
      Widget host(Widget child) => UncontrolledProviderScope(
        container: container,
        child: MaterialApp(
          home: Theme(data: AppTheme.light(), child: child),
        ),
      );

      await tester.pumpWidget(
        host(
          const ChatThreadScreen(conversation: _kChannel, pollInterval: null),
        ),
      );
      await tester.pump();
      await tester.pump();
  ...
  testWidgets(
    'revenir dans une conversation deja vue n’affiche plus de squelette',
    (tester) async {
      final container = ProviderContainer(
        overrides: [
          authControllerProvider.overrideWith(_FakeAuth.new),
  ...
  testWidgets('renders mine vs others bubbles, author name, edited + deleted', (
    tester,
  ) async {
    ...
    expect(
      find.text('Awa Diallo'),
      findsWidgets,
    ); // author name on group bubble
    expect(find.textContaining('modifié'), findsOneWidget); // edited indicator
    expect(
      find.textContaining('supprimé'),
      findsOneWidget,
    ); // deleted indicator
  ...
  testWidgets('shows the empty state when there are no messages', (
    ...
    await tester.pump();
    expect(
      find.text('Aucun message, démarrez la conversation'),
      findsOneWidget,
    );
    await tester.pumpWidget(const SizedBox());
  });
  ...
  testWidgets('loading older messages refetches with the next cursor', (
    tester,
  ) async {
    final repo = _MessagesRepo();
    await tester.pumpWidget(_host(repo));
  ...
  testWidgets(
    'the bubble appears before the server answers, then gets a tick',
    (tester) async {
  ...
  testWidgets(
    'renders a tick per delivery state, and none on others’ messages',
    (tester) async {
  ...
  testWidgets('delete-for-everyone past 24h surfaces the 422 message', (
    tester,
  ) async {
  ...
  testWidgets('never subscribes to the realtime channel itself', (
    tester,
  ) async {
